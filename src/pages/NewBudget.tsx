...
const NewBudget = () => {
      ...
      try {
        ...
        try {
          ...
          if (orcamentoSnap.exists()) {
            ...
            const itensProcessados = (orcamentoData.itens || []).map(item => {
              // Garantir que cada item tem um ID
              if (!item.id) {
                return { ...item, id: `item_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` };
              }
              return item;
            });
  ...
  const handleGerarPDF = async () => {
    ...
    try {
      ...
      const orcamento: Orcamento = {
        ...
        data: new Date(),
        valor: calcularTotal(),
        status: 'Pendente' // Status padrão para orçamentos
      };
      ...
      // Gerar o PDF
      const pdfBlob = await generatePDF(orcamento, null);
  ...
  const handleEnviarEmail = async () => {
    ...
    try {
      ...
      await generatePDF(orcamento, null);
